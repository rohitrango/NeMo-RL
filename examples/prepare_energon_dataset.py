...
Example::
    ...
    uv run examples/prepare_energon_dataset.py \
        --splits train valA
    ...
    with wds.ShardWriter(str(pattern), maxcount=max_samples_per_shard) as writer:
        for index, example in enumerate(examples):
            if max_samples is not None and count >= max_samples:
                break
            image_bytes, image_extension, width, height = (
                _image_bytes_extension_and_size(example.get("image"))
            )
            writer.write(
                {
...
def prepare_clevr_energon(
    *,
    ...
    for split in split_names:
        logger.info("Converting CLEVR-CoGenT split %s", split)
