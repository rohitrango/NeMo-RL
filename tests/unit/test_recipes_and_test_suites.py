...
def test_nightly_compute_stays_below_4157_hours(nightly_test_suite, tracker):
    ...
    # Reserve 8 GPU-hours for the SingleController MOPD nightly on top of
    # main's 4149-hour limit.
    assert total_gpu_hours <= 4157, (
        f"Total GPU hours exceeded 4157: {last_line}. We should revisit the test suites to reduce the total GPU hours."
    )
