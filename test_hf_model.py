...
uv run --env-file .env --extra mcore tools/model_diagnostics/3.check_hf_model_embeddings_untrained.py 
uv run --env-file .env --extra mcore tools/model_diagnostics/3.check_hf_model_embeddings_untrained.py --model /tmp/nvidia_Nemotron-H-8B-Base-8K_modified
uv run --env-file .env --extra mcore tools/model_diagnostics/3.check_hf_model_embeddings_untrained.py --model /lustre/fs1/portfolios/llmservice/users/adithyare/nano_v2_evals_fixed_template_try2/bkartal/projects/nano_v2/experiments/nano-v2-sft-en-m-lingual-20-percent-stage1-lr5e-6/checkpoints/nano-v2-sft-en-m-lingual-20-percent-stage1-lr5e-6/iter_0019000/evals/hf/hf/
"""
...
def print_problematic_embeddings(weights, indices, problem_type, metric_values, threshold, tokenizer=None):
    ...
    for idx in indices:
        ...
        print(f"Index {idx}: {problem_type} (metric: {metric_val:.2e} > {threshold:.2e})")
        print(f"  Token: {token_info}")
        print(f"  Values: [{first_two[0]:.2e}, {first_two[1]:.2e}, ..., {last_two[0]:.2e}, {last_two[1]:.2e}]")
...
def check_embedding_layer(embeddings, layer_name, near_zero_threshold, identical_threshold, tokenizer=None, model=None):
    ...
    weights = embeddings.weight.data if hasattr(embeddings, 'weight') else embeddings.data
    ...
    print_problematic_embeddings(weights, near_zero_indices, "Near-zero", max_abs_values, near_zero_threshold, tokenizer)
    print_problematic_embeddings(weights, identical_indices, "Identical", row_stds, identical_threshold, tokenizer)
...
def main():
    parser = argparse.ArgumentParser(description="Check HuggingFace model embeddings for untrained patterns")
    parser.add_argument("--model", 
                       default="nvidia/Nemotron-H-8B-Base-8K",
                       help="HuggingFace model name or path")
    parser.add_argument("--near-zero-threshold", 
                       type=float, default=1e-10,
                       help="Threshold for detecting near-zero embeddings (default: 1e-10)")
    parser.add_argument("--identical-threshold", 
    ...
    try:
        ...
        model = AutoModelForVision2Seq.from_pretrained(args.model, torch_dtype='auto', trust_remote_code=True)
    ...
    if input_embeddings is not None:
        input_summary = check_embedding_layer(input_embeddings, "Input Embeddings", args.near_zero_threshold, args.identical_threshold, tokenizer, model)
    ...
    if output_embeddings is not None:
        output_summary = check_embedding_layer(output_embeddings, "Output Embeddings", args.near_zero_threshold, args.identical_threshold, tokenizer, model)
    ...
    for summary in summaries:
        ...
        print(f"Near-zero embeddings (abs < {summary['near_zero_threshold']:.2e}): {summary['num_near_zero']}/{summary['total_embeddings']} ({100*summary['num_near_zero']/summary['total_embeddings']:.1f}%)")
        if summary['near_zero_indices']:
            print(f"  Indices: {format_index_ranges(summary['near_zero_indices'])}")
        
        print(f"Identical embeddings (std < {summary['identical_threshold']:.2e}): {summary['num_identical']}/{summary['total_embeddings']} ({100*summary['num_identical']/summary['total_embeddings']:.1f}%)")
        if summary['identical_indices']:
            print(f"  Indices: {format_index_ranges(summary['identical_indices'])}")
        
        print(f"Statistics: mean_abs={summary['mean_abs']:.6f}, max_abs={summary['max_abs']:.6f}, std_range=[{summary['min_std']:.6f}, {summary['max_std']:.6f}]")
